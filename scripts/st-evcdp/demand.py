"""
File: EV Battery Swapping Demand Calculation and Data Processing for ST-EVCDP Dataset
...
Outputs:
- output/st-evcdp/demand.csv: The final processed data with battery replacement demand.
"""
...
volume_df = read_csv_with_progress('datasets/volume.csv')
...
for area in tqdm(areas, desc="Processing areas"):
    ...
    segment_count = 0
    area_results = []  # Store results for the current area
    while i < len(duration_col):
        ...
        if current_duration > 0:
            ...
            if next_index < len(duration_col) and duration_col[next_index] >= duration_col[next_index - 1]:
                ...
                temp = current_duration * 60 / 5 - (duration_col[next_index] * 60 / 5)
